...
void MksAxisWorker::handle_service_queue_command(const motion_core::ServiceCommandPoint& command) {
    ...
    switch (command.type) {
        ...
        case motion_core::ServiceCommandType::Disable:
            // Audit Bug 7: Clear motion queue on MKS Disable to prevent stale motion
            clear_motion_queue();
...
motion_core::Result<MksBusCommand> MksAxisWorker::build_absolute_command(
    ...
    if (std::isfinite(point.velocity) && point.velocity > 0.0) {
        // Audit Bug 4: Fix speed unit mismatch by converting load deg/s to motor RPM
        const double ratio = (context.software_gear_ratio > 0.0 && std::isfinite(context.software_gear_ratio))
            ? context.software_gear_ratio : 1.0;
        const double motor_deg_s = point.velocity * ratio;
        const double rpm = std::abs(motor_deg_s) / 6.0;
        speed_rpm = static_cast<std::uint16_t>(std::clamp<std::uint32_t>(
