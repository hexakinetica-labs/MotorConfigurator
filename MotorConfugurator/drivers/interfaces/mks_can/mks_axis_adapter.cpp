...
motion_core::Result<void> MksAxisAdapter::apply_parameter_patch(const motion_core::ParameterPatch& patch) {
    ...
    if (config_.bus_manager) {
        const auto recovery_result = config_.bus_manager->request_telemetry_recovery(
            runtime_can_id_.load(std::memory_order_acquire));
...
void MksAxisAdapter::process_homing_state_machine(const std::chrono::steady_clock::time_point now) {
    ...
    if (output.failed) {
        ...
        } else if (current == HomingStatus::ApplyingSetZero) {
            // Audit Bug 5: Only transition to Completed after position readback confirms zero reset (< 0.1 deg)
            if (std::abs(telemetry.position) < 0.1) {
    ...
    if (output.request_set_zero) {
        ...
        // Audit Bug 5: transition to ApplyingSetZero instead of Completed immediately.
        // It will transition to Completed in the step loop once position readback is close to 0.
        set_homing_status(HomingStatus::ApplyingSetZero);
    }
...
bool MksAxisAdapter::config_busy() const {
    return state_cache_.config_busy();
}
