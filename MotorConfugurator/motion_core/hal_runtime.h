...
private:
    // Fast path used during runtime startup to avoid expensive readback.
    // It still filters by descriptor metadata, and preserves the "late" ordering for transport-breaking params.
    [[nodiscard]] Result<ParameterPatch> build_axis_config_patch_for_apply(std::uint16_t axis_id, const std::string& path) const;
