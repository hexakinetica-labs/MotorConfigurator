...
Result<void> HalRuntime::start() {
    ...
    const auto rollback_started = [&started_buses, &started_axes]() {
        ...
        // Bugs 3 & 3.1: Sleep to allow Disable commands to drain to the hardware before stopping the buses
    ...
    for (const auto& [id, axis] : axes_snapshot) {
        ...
        // Audit Bug 6: Disable/remove 'enable_on_start' policy at low-level startup
        // to prevent energizing axes before the startup safety baseline disables them.
        /*
...
Result<ParameterPatch> HalRuntime::build_axis_config_patch_for_apply(const std::uint16_t axis_id, const std::string& path) const {
    const auto axis_res = find_axis(axis_id);
    if (!axis_res.ok()) {
        return Result<ParameterPatch>::failure(axis_res.error());
    }
...
Result<void> HalRuntime::apply_axis_config_file(const std::uint16_t axis_id, const std::string& path) {
    const auto patch_res = build_axis_config_patch_for_apply(axis_id, path);
    if (!patch_res.ok()) {
