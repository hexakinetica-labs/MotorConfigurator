...
motion_core::Result<ControlOp> control_op_from_string(const std::string& value) {
    ...
    // Accept legacy ops gracefully (backwards compat with older HexaMotion clients)
    if (value == "RequestManualLease" || value == "ReleaseManualLease") {
        return motion_core::Result<ControlOp>::success(ControlOp::None);
    }
...
motion_core::Result<OwnerRole> owner_role_from_string(const std::string& value) {
    ...
    // Accept legacy "Service" role gracefully
    if (value == "Service") return motion_core::Result<OwnerRole>::success(OwnerRole::None);
