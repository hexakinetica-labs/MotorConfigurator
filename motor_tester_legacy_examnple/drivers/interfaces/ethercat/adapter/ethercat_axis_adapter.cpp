...
constexpr double kManualHomingMotorVelocityDegPerSec = 100.0;
constexpr std::uint32_t kManualHomingDi3Mask = (1U << 2U);
...
motion_core::Result<void> EthercatAxisAdapter::configure_hardware() {
    ...
    std::array<unsigned int, 10> bit_positions{};
...
motion_core::Result<void> EthercatAxisAdapter::start() {
    ...
    cmd_.manual_homing_active.store(false, std::memory_order_release);
...
motion_core::Result<void> EthercatAxisAdapter::set_mode(motion_core::AxisMode mode) {
    ...
    if (mode != motion_core::AxisMode::ManualHoming) {
        cmd_.manual_homing_active.store(false, std::memory_order_release);
    }
...
motion_core::Result<void> EthercatAxisAdapter::apply_command(const motion_core::AxisCommand& command) {
    ...
    if (command.go_home) {
        ...
        if (mode == motion_core::AxisMode::ManualHoming) {
            cmd_.mode_req.store(mode_to_work_mode(motion_core::AxisMode::ManualHoming), std::memory_order_release);
            cmd_.manual_homing_active.store(true, std::memory_order_release);
            cmd_.homing_req.store(false, std::memory_order_release);
            cmd_.homing_active.store(false, std::memory_order_release);
    ...
    if (command.has_profile_speed_rpm) {
        ...
        if (mode == motion_core::AxisMode::ProfilePosition) {
            ...
            const double rps = static_cast<double>(command.profile_speed_rpm) / 60.0;
            const double counts_per_rev_output =
                static_cast<double>(counts_per_revolution_.load(std::memory_order_acquire)) * gear_ratio;
            const auto profile_vel_instr_s_u64 = static_cast<std::uint64_t>(std::llround(std::max(
                0.0,
                rps * counts_per_rev_output)));
            const auto profile_vel_instr_s = static_cast<std::uint32_t>(std::min<std::uint64_t>(
                profile_vel_instr_s_u64,
                static_cast<std::uint64_t>(std::numeric_limits<std::uint32_t>::max())));

            std::vector<std::uint8_t> payload(sizeof(profile_vel_instr_s), 0U);
            std::memcpy(payload.data(), &profile_vel_instr_s, sizeof(profile_vel_instr_s));
            const auto write_status = request_sdo_write(ObjProfileVelocity, 0x00, payload);
            if (!write_status.ok()) {
                return write_status;
            }
        }
...
motion_core::Result<void> EthercatAxisAdapter::clear_motion_queue() {
    clear_motion_queue_req_.store(true, std::memory_order_release);
    has_queued_target_.store(false, std::memory_order_release);
    motion_queue_was_populated_.store(false, std::memory_order_release);
    return motion_core::Result<void>::success();
}
...
void EthercatAxisAdapter::process_cycle(uint8_t* domain_pd, double dt_s) {
    ...
    bool manual_homing_active = cmd_.manual_homing_active.load(std::memory_order_acquire);
    int8_t mode = cmd_.mode_req.load(std::memory_order_acquire);

    const auto apply_homing_zero_from_current_position = [&]() {
        const double shift_deg = home_switch_to_zero_shift_deg_.load(std::memory_order_acquire);
        const double shift_rad = shift_deg * (M_PI / 180.0);
        const auto shift_counts_i64 = static_cast<std::int64_t>(std::llround(shift_rad * counts_per_radian));
        const auto new_zero_offset_i64 = static_cast<std::int64_t>(act_pos_counts) + shift_counts_i64;
        const auto new_zero_offset = static_cast<std::int32_t>(std::clamp<std::int64_t>(
            new_zero_offset_i64,
            static_cast<std::int64_t>(std::numeric_limits<std::int32_t>::min()),
            static_cast<std::int64_t>(std::numeric_limits<std::int32_t>::max())));
        zero_offset_counts_.store(new_zero_offset, std::memory_order_release);
    };
    ...
    const bool homing_attained = (statusword & MaskHomingAttained) != 0U;
    if (homing_attained && homing_active) {
        apply_homing_zero_from_current_position();
    }
    if (homing_attained) {
        homing_active = false;
    }

    if (manual_homing_active) {
        mode = 3; // Profile Velocity
    }

    if (mode != 3 || !enable_req || is_faulted) {
        manual_homing_active = false;
    }

    const bool manual_homing_switch_hit = (digital_inputs & kManualHomingDi3Mask) != 0U;
    if (manual_homing_active && manual_homing_switch_hit) {
        apply_homing_zero_from_current_position();
        manual_homing_active = false;
    }

    if (mode == 6 && homing_active && is_enabled) {
        controlword |= kControlwordOperationStartBit; // Homing Operation Start (Bit 4)
    }

    if (mode == 1 && enable_req && !is_faulted) {
    ...
    cmd_.manual_homing_active.store(manual_homing_active, std::memory_order_release);
    ...
    if (mode == 8 || mode == 9) {
        ...
        if (motion_queue_.try_pop(queued)) {
            has_new_target = true;
            // Track that the queue has been populated and consumption has begun
            if (!motion_queue_was_populated_.load(std::memory_order_acquire)) {
                // First pop ever since clear — check if prefill was adequate
                const auto remaining = motion_queue_.size_approx();
                if (remaining < 20U) {
                    // Started consuming before buffer was adequately pre-filled
            ...
            // Queue was previously populated but is now empty — underrun
    ...
    if (manual_homing_active) {
        ...
        velocity_command_deg_s = kManualHomingMotorVelocityDegPerSec;
    } else if (has_velocity_command) {
...
int8_t EthercatAxisAdapter::mode_to_work_mode(motion_core::AxisMode mode) const {
    switch (mode) {
        ...
        case motion_core::AxisMode::ManualHoming: return 3;       // PV (manual homing below AxisManager)
