...
public slots:
    ...
    // Direct enqueue fast-path for UI streaming.
    // Internally routed through a shared ingress with source guard policy.
    ...
    // Direct queue-stats fast-path for UI monitoring.
    // Uses shared ingress observer path (no motion ownership gate).
...
private:
    motion_core::Result<void> ensureManualMotionQueueReady(int axis_id);
    bool isReady() const;
