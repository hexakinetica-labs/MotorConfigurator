...
motion_core::Result<void> HalIpcClient::send_control_frame(const HalControlFrameDto& frame) {
    const auto state = exchange_control_frame(frame);
    if (!state.ok()) {
        return motion_core::Result<void>::failure(state.error());
    }
    return motion_core::Result<void>::success();
}
